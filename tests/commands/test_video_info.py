...
def test_execute_with_ids_and_urls(youtube_mock, mocker, tmp_path, mock_video_info):
    ...
    urls = [
        "https://www.youtube.com/watch?v=tmrhPou85HQ&ab_channel=Turicas",
        "https://www.youtube.com/watch?v=qoI_x9fylaw&ab_channel=PythonicCaf%C3%A9",
    ]
    output_file_path = tmp_path / "output.csv"
    VideoInfo.execute(ids=ids, urls=urls, output_filename=output_file_path, api_key="test_api_key")
    assert output_file_path.exists()
    with output_file_path.open(mode="r") as f:
...
def test_execute_with_input_file_path(youtube_mock, mocker, tmp_path, mock_video_info):
    ...
    VideoInfo.execute(
        input_file_path=str(input_file_path), output_filename=output_file_path, api_key="test_api_key"
    )
    assert output_file_path.exists()
    with output_file_path.open(mode="r") as f:
        reader = csv.DictReader(f)
...
def test_execute_with_info_columns(youtube_mock, mocker, tmp_path, mock_video_info):
    ...
    ids = ["tmrhPou85HQ", "qoI_x9fylaw"]
    output_file_path = tmp_path / "output.csv"
    VideoInfo.execute(ids=ids, output_filename=output_file_path, api_key="test_api_key", info_columns="id,title")
    assert output_file_path.exists()
    with output_file_path.open(mode="r") as f:
        reader = csv.DictReader(f)
        csv_data = list(reader)
